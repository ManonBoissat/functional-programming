    ...
    def union(that: TweetSet): TweetSet = {
      def unionAcc(that: TweetSet, acc:TweetSet): TweetSet = {
        if(that.isEmpty)
          ...
          unionAcc(that.tail, acc.incl(that.head))
      }
      unionAcc(that, this)
    }
   ...
   */
    def mostRetweeted: Tweet = {
      def mostRetweetedAcc(acc: Tweet): Tweet = {
        if(this.isEmpty) acc
        else if(acc.retweets < this.tail.head.retweets)
          mostRetweetedAcc(this.tail.head)
        else
          mostRetweetedAcc(acc)  
      }
      mostRetweetedAcc(this.head)
    }
   ...
   */
    def descendingByRetweetAcc(acc:TweetList):TweetList = {
      if(this.isEmpty) acc
        ...
        this.remove(max).descendingByRetweetAcc(acc + max)
      }
    }
