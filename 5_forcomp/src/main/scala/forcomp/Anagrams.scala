...
object Anagrams {
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iterSentence(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
      else
        for {
          combination <- combinations( occurrences )
          word <- dictionaryByOccurrences.getOrElse(combination, Nil)
          sentence <- iterSentence( subtract(occurrences,wordOccurrences(word)) )
          if !combination.isEmpty
      } yield word :: sentence
    }

    iterSentence( sentenceOccurrences(sentence) )
  }
}
