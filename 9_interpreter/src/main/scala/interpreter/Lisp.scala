...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'def :: (name :: args) :: body :: expr =>
      eval('def :: name :: ('lambda :: args :: body :: Nil) :: expr, env)
    
    // rest = rest du programme
    // (case 1 (else 2)) == 2
    // (case 1 (1 3) (else 4)) == 3
    case ('case :: expr_scrut :: values) => values match {
      case ('else :: expr_else :: Nil) :: Nil =>
        eval(expr_else , env)  
        
      case (value :: expr :: Nil) :: tail =>
        eval('if :: ('= :: expr_scrut :: List(value)) :: expr :: ('case :: expr_scrut :: tail) :: Nil, env)
